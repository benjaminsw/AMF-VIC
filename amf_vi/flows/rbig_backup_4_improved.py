...
class MarginalGaussianization(nn.Module):
    ...
    def _cdf_and_pdf(self, x, x_vals, cdf_vals, eps=1e-12):
        ...
        idx = torch.searchsorted(x_vals.contiguous(), x.contiguous(), right=False).clamp(1, len(x_vals)-1)
...
class PCARotation(nn.Module):
    """PCA-based rotation matrix computed from data."""
    
    def __init__(self, dim: int):
        super().__init__()
        self.dim = dim
        
        # Initialize with identity - will be replaced when fitted
        self.register_buffer('rotation_matrix', torch.eye(dim))
        self.is_fitted = False
    
    def fit_pca(self, x: torch.Tensor):
        """Compute PCA rotation matrix from data."""
        # Center the data
        ...
        # Sort by eigenvalues (descending) and use eigenvectors as rotation matrix
...
class RBIGFlow(BaseFlow):
    ...
    def __init__(self, dim: int, n_layers: int = 4, n_bins: int = 1000):
        ...
        self.rotation_layers = nn.ModuleList([
            PCARotation(dim) for _ in range(n_layers)
        ])
    ...
    def fit_to_data(self, x: torch.Tensor, validate_reconstruction: bool = True):
        """Fit the empirical CDFs and PCA rotations to training data with optional validation."""
        current_x = x.clone()
        ...
        for i in range(self.n_layers):
            ...
            # Fit PCA rotation on Gaussianized data
            self.rotation_layers[i].fit_pca(current_x)
            
            # Apply rotation to get input for next layer
            with torch.no_grad():
